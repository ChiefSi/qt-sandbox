...
QSize ConfigTreeItemDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
	if (index.isValid())
	{
		...
		int textHeightEstimate = qMax(option.fontMetrics.ascent(), option.fontMetrics.descent());

		QSize hint(textSize.width() + 4 + iconSize.width(),
...
void ConfigTreeItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
	//if (index.isValid())
	//{
		painter->save();
		...
		style->drawPrimitive(QStyle::PE_PanelItemViewItem, &styleOption, painter, styleOption.widget);
		...
		/*
		QRect alignedRect = QStyle::alignedRect(styleOption.direction, Qt::AlignHCenter | Qt::AlignLeft, option.rect.size(), option.rect);
		styleOption.icon.paint(painter, alignedRect);
		*/
		int height = option.fontMetrics.lineSpacing();
		QRect textRect(option.rect.left(), option.rect.bottom() - height, option.rect.width(), height);
